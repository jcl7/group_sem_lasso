...
#################
# POCT HOC ANALYSIS 2 - With the more strict lambda selection rule.
# The "one-standard-error" rule (elem stat learn p 244).
...
for (nn in 1:length(nns)){
	for (ii in 1:100){
		...
		cv_sd_error <- apply(sim_cv_res_avt$lambda_fits_cv[,2,], MARGIN = 1, FUN=sd)
		
		best_cv_lamba <- sim_cv_res_avt$lambdas_cv[which.min(cv_mean_error)]
		
		# the maximim error allowd for a lambda
		# The least CV error + 1 std error. (aka the one standard error rule
		max_error_lambda <- cv_mean_error[which.min(cv_mean_error)] + cv_sd_error[which.min(cv_mean_error)]
		
		# Find the largest lambda that is within 1 se of the
		best_cv_lamba_1se <- max(sim_cv_res_avt$lambdas_cv[cv_mean_error <= max_error_lambda])
		
		
		est_lasso_avt_best_lambda_1se <- estimate_ram_group_lasso(ram_matrices_avt,
																															inndata = simdta_scaled,
																															group_var = 'group',
																															lambda_values = best_cv_lamba_1se,
																															verbose=FALSE)
		
		
		get_nice_par_table(est_lasso_avt_best_lambda_1se, which_group=2) %>% 
			mutate(N = nns[nn],
						 simno = ii) %>% 
...
#save(all_param_ests, file='all_param_ests_h1_20180625_1se.rdata')
