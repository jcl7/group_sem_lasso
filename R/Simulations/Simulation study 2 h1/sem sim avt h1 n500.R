...
sim_nobs <- 500*2 # total number of observations
nsim <- 100


seed_file <- 'sim_seeds_08.txt' # todo
...
avt_res <- sem(avt.model1, fixed.x = FALSE, group = 'group', data = dta_scaled)
summary(avt_res)
...
# set all parameters in AA the same as in EA.
mypar$ustart[mypar$group == 2] <- mypar$ustart[mypar$group == 1]

# the parameters that should be as they are.
idx1 <- mypar$group == 2 & mypar$lhs == 'actuhap' & mypar$rhs == 'cultatt'
mypar$ustart[idx1] <- mypar$est[idx1]

idx2 <- mypar$group == 2 & mypar$lhs == 'actuhap' & mypar$rhs == 'temperatt'
mypar$ustart[idx2] <- mypar$est[idx2]

idx3 <- mypar$group == 2 & mypar$lhs == 'rigoract' & mypar$rhs == 'actuhap'
mypar$ustart[idx3] <- mypar$est[idx3]
...
# set.seed(23942)
# sim_nobs <- 5000
# simdta <- simulateData(mypar, fixed.x = FALSE, sample.nobs = rep(sim_nobs/2, 2))
# simfit <- sem(avt.model1, fixed.x = FALSE, group = 'group', data = simdta)
# 
# mypar_sim <- parTable(simfit)
# 
# plot(mypar_sim$est, mypar$est, asp=1) # should not agree completely.
# plot(mypar_sim$est, mypar$ustart, asp=1) # should agree.
# 
# abline(coef=c(0,1))
